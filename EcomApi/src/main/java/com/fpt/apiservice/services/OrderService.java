...
import com.fpt.apiservice.models.Orders.OrderStatus;
import com.fpt.apiservice.repositories.OrderRepository;
import com.fpt.apiservice.requests.Orders.OrderRequest;
import com.fpt.apiservice.requests.Orders.ProductResponse;
...
public class OrderService {
    ...
    public void checkExistProducts(OrderRequest request) {
        ...
        List<Long> idList = request.products().stream()
    ...
    public ResponseEntity<Order> createOrder(OrderRequest request, Long userId) {
        //check order request
    ...
    public ResponseEntity<Order> updateOrder(String orderId, OrderRequest request, Long userId) {
        Optional<Order> orderData = orderRepository.findById(orderId);
        if (orderData.isEmpty()) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
        ...
        if (request.status() != null) {
            orderData.get().setStatus(request.status());
        }
